...
function ProjectCard({project,index,onOpen,onAura,featured=true}){
  return <Reveal className={featured?'projectCardReveal featured':'projectCardReveal'}>
    <article className={`projectCard project-${project.id} ${featured?'featured':''}`} onMouseEnter={()=>onAura?.(project.id)} onMouseLeave={()=>onAura?.('default')}>
...
const photoPortfolio=[
  {src:'photography/E718FCA3-33F8-42CC-BA38-D7F94D9C42B4.jpeg',alt:'San Francisco skyline framed by a large tree above the Painted Ladies',place:'San Francisco'},
  {src:'photography/D72E7871-553A-4E7A-AF5B-1D095B0C26F1.jpeg',alt:'Colorful double exposure of Coit Tower and a flower',place:'San Francisco'},
  {src:'photography/D4A21105-DBC1-4A61-8D02-C3F4C9FBC0F2.jpeg',alt:'Double exposure of the Golden Gate Bridge with hanging objects',place:'Golden Gate Bridge'},
  {src:'photography/74046AD1-4BB8-4C22-A625-44963F19181A.jpeg',alt:'Golden Gate Bridge disappearing into fog above traffic',place:'San Francisco'},
  {src:'photography/A11E4F92-2869-472B-A154-624514BDB1D8.jpeg',alt:'Small colorful boats floating in the Bay under a blue sky',place:'Bay Area'}
];
function PhotographyPage({onBack}){
  const [lightbox,setLightbox]=useState(null);
  return <main className="photoPage"><AuraField tone="photography"/><header className="photoPageHeader"><button type="button" onClick={onBack}>← Portfolio</button><span>Film photography</span></header><section className="photoIntro"><p>Film photography</p><h1>Things I wanted to keep.</h1><span>City scenes, coastlines, color, and the occasional double exposure.</span></section><section className="photoGrid" aria-label="Film photography portfolio">{photoPortfolio.map((photo,i)=><button type="button" className={`photoTile photoTile-${i+1}`} key={photo.src} onClick={()=>setLightbox(photo)}><img loading="lazy" decoding="async" src={photo.src} alt={photo.alt}/><span>{photo.place}</span></button>)}</section><section className="photoPlaces"><div><span>Places I keep coming back to</span><strong>San Francisco · the Bay · wherever I am walking</strong></div><p>I shoot mostly on 35mm film and usually carry a camera when I am exploring somewhere new.</p></section><ImageLightbox image={lightbox} onClose={()=>setLightbox(null)}/></main>
}

const toolLogoMap={
  react:['react','React'], mui:['mui','Material UI'], git:['git','Git'], github:['github','GitHub Projects'], typescript:['typescript','TypeScript'], javascript:['javascript','JavaScript'], html:['html5','HTML'], css:['css3','CSS'], figma:['figma','Figma'], kotlin:['kotlin','Kotlin'], spring:['spring','Spring'], excel:['microsoftexcel','Microsoft Excel'], powerpoint:['microsoftpowerpoint','PowerPoint'], powerbi:['powerbi','Power BI'], openai:['openai','OpenAI'], aem:['adobeexperiencecloud','Adobe Experience Manager'], salesforce:['salesforce','Salesforce'], postman:['postman','Postman'], jenkins:['jenkins','Jenkins'], servicenow:['servicenow','ServiceNow'], splunk:['splunk','Splunk'], flask:['flask','Flask'], mysql:['mysql','MySQL'], socketio:['socketdotio','Socket.IO'], docker:['docker','Docker'], gcp:['googlecloud','Google Cloud'], python:['python','Python']
};
const toolSets={fcvf:['react','typescript','javascript','html','css','mui','figma','kotlin','spring','git','github','excel'],finsimple:['aem','salesforce','postman','jenkins','servicenow','splunk'],scheduler:['flask','mysql','socketio','docker','gcp','javascript'],chat:['flask','socketio','javascript'],estee:['javascript','react','git','figma'],accenture:['salesforce','excel','powerpoint','powerbi','openai']};
function ToolLogoStrip({id}){const items=toolSets[id]||[];if(!items.length)return null;return <section className="toolLogoSection"><h2>Tech Stack</h2><div className="toolLogoRow">{items.map(k=>{const [slug,name]=toolLogoMap[k];return <span className="toolLogoWrap" key={k} data-tool={name}><img loading="lazy" decoding="async" src={`https://cdn.simpleicons.org/${slug}/3F3D3A`} alt={name} title={name}/></span>})}</div></section>}
...
function CaseStudy({id,onBack}){
 ...
 {id==='estee'&&<><CaseSection title="Customer / User"><p>Online foundation shoppers who need enough product understanding and shade confidence to move toward purchase.</p></CaseSection><CaseSection title="Night Hack"><p>For an Estée Lauder × Kode With Klossy challenge, I worked on the product concept, UX/UI, and frontend for a Double Wear discovery experience. The team finished as a Top 5 finalist, and I presented the concept to Estée Lauder C-suite leadership.</p></CaseSection><CaseSection title="What I made"><p>I focused the experience on connecting product education, shade exploration, and a clear path to purchase while keeping the interface recognizably Estée Lauder.</p><EsteeVisual/></CaseSection></>}
...
function MoreProjectCard({project,onOpen}){
  return <article className={`moreBuildCard clickable project-${project.id}`}><button className="moreBuildAction" type="button" onClick={()=>onOpen(project.id)} aria-label={`Open ${project.title}`}><div className="moreBuildVisual"><ProjectCover type={project.media}/></div><div className="moreBuildCopy"><span>{project.company}</span><h3>{project.title}</h3><span className="projectTextLink">View →</span></div></button></article>
}
