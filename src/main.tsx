...
function CommutePreview(){
  return <div className="commutePreview commutePreviewV39"><div className="commutePreviewGlow" aria-hidden="true"></div><div className="commutePreviewDevice"><div className="iosPhoneDevice commutePreviewDeviceFrame"><div className="iosDemoShell commutePreviewShell"><div className="iosStatus previewStatusBar"><span>7:22</span><PreviewStatusIcons/></div><div className="iosScreenContent"><CommutePreviewScreen/></div><div className="iosHomeIndicator" aria-hidden="true"></div></div><img className="iosHardwareFrame" src="project-media/iphone-frame-v31.png" alt="" aria-hidden="true"/></div></div></div>
}
...
function PhotoCollectionCarousel({section,onExpand}){
  ...
  return <section id={section.id} className="photoCollectionSection photoCarouselSection"><div className="photoCollectionMeta"><p>{section.note}</p><h2>{section.title}</h2><span>{section.description}</span></div><div className={`photoCarousel photo-${orientation}`} onTouchStart={e=>{touchStart.current=e.touches[0]?.clientX??null}} onTouchEnd={e=>{if(touchStart.current===null)return;const x=e.changedTouches[0]?.clientX??touchStart.current;const delta=x-touchStart.current;if(Math.abs(delta)>45)move(delta<0?1:-1);touchStart.current=null}}><button type="button" className={`photoCarouselStage ${orientation}`} onClick={()=>onExpand(section.photos[index])} aria-label={`Open ${section.title} photo ${index+1}`}><img key={section.photos[index].src} onLoad={setImageOrientation} src={section.photos[index].src} alt={section.photos[index].alt}/></button><div className="photoCarouselControls"><div><button type="button" onClick={()=>move(-1)} aria-label={`Previous ${section.title} photo`}>←</button><button type="button" onClick={()=>move(1)} aria-label={`Next ${section.title} photo`}>→</button></div><span>{String(index+1).padStart(2,'0')} / {String(count).padStart(2,'0')}</span></div><div className="photoFilmstrip" aria-label={`${section.title} thumbnails`}>{section.photos.map((photo,i)=><button type="button" className={i===index?'active':''} key={photo.src} onClick={()=>setIndex(i)} aria-label={`Show ${section.title} photo ${i+1}`} aria-current={i===index?'true':undefined}><img loading="lazy" decoding="async" src={photo.src} alt=""/></button>)}</div></div></section>
}
function PhotographyPage({onBack}){
  ...
  return <main className="photoPage photoGalleryPage"><AuraField tone="photography"/><header className="photoPageHeader"><button type="button" onClick={onBack}>← Portfolio</button><span>Photography</span></header><section className="photoGalleryIntro"><div className="photoGalleryCopy"><p>35mm film</p><h1>Selected work.</h1><span>Michigan, San Francisco, Lake Tahoe, Yosemite, Hawaii, and Chicago.</span><small>A rotating set of photographs from places I wanted to remember.</small></div><figure className="photoHeroShot"><img loading="eager" decoding="async" src={featured.src} alt={featured.alt}/></figure></section><nav className="photoCollectionNav" aria-label="Photography collections">{photoCollections.map(section=><button type="button" key={section.id} onClick={()=>jumpTo(section.id)}>{section.title}</button>)}</nav><section className="photoGalleryStatement"><p>Browse each set one photograph at a time. Swipe on mobile or use the arrows and thumbnails.</p></section>{photoCollections.map(section=><PhotoCollectionCarousel key={section.id} section={section} onExpand={setLightbox}/>)}<ImageLightbox image={lightbox} onClose={()=>setLightbox(null)}/></main>
}
...
function CaseStudy({id,onBack}){
 ...
 return <main className={`casePage case-${id}`} onClick={openClickedImage}><AuraField tone={id}/><button className="backBtn" onClick={onBack}>← Projects</button><section className="caseLead"><header className="caseHeader"><CaseCompanyBar id={id} fallback={p.company}/><h1>{p.title}</h1><div className="caseIntro">{p.summary}</div>{id!=='commute'&&<div className="ownershipLine"><span>{ownership[id]}</span></div>}{id!=='commute'&&metrics[id]&&<MetricStrip items={metrics[id]}/>}</header><div className="caseHeroMedia casePreviewHero"><ProjectVisual type={p.media}/></div></section><ToolLogoStrip id={id}/>
 {id==='commute'&&<><section className="sandboxSection commuteShowcase"><CommuteSandbox/></section><CaseSection title="Users"><p><strong>First user:</strong> me. <strong>Broader user:</strong> anyone with a recurring commitment whose morning depends on several changing inputs.</p></CaseSection><CaseSection title="Why I built it"><p>I got tired of doing the same morning calculation over and over again. Commute works backward from where I need to be, then combines my routine with live transit, traffic, walking time, and missed-departure risk to tell me when to wake up and when to leave.</p><p>Maps can tell me how to get somewhere. I wanted the decision before that: when does my morning actually need to start?</p></CaseSection><section className="caseSection caseNarrative"><h2>The model starts at the destination.</h2><div><div className="decisionSteps"><div><span>1</span><strong>Arrive</strong><p>Calendar commitment + chosen arrival buffer.</p></div><div><span>2</span><strong>Travel</strong><p>Candidate routes + walking + transfers + missed-departure cost.</p></div><div><span>3</span><strong>Check reality</strong><p>Live arrivals + traffic + service alerts + source freshness.</p></div><div><span>4</span><strong>Get ready</strong><p>Personal routine + observed walking behavior.</p></div><div><span>5</span><strong>Act</strong><p>Wake time + latest safe departure + recommended mode.</p></div></div></div></section><section className="caseSection technicalNote"><h2>Native architecture + integrations</h2><div><div className="dataSourceList"><div><strong>Google Routes</strong><span>walking + route geometry</span></div><div><strong>511 / GTFS-Realtime</strong><span>BART + AC Transit arrivals, vehicles, alerts</span></div><div><strong>511 traffic</strong><span>road incidents + bridge conditions</span></div><div><strong>HealthKit</strong><span>walking pace</span></div><div><strong>Calendar</strong><span>time + place</span></div><div><strong>Weather</strong><span>walking and road-context adjustment</span></div><div><strong>AlarmKit / iOS alarms</strong><span>wake-time execution in a native build</span></div><div><strong>Observed commute history</strong><span>route-specific walking and prediction error over time</span></div></div></div></section><section className="caseSection decisionEditorial"><h2>Choices I made</h2><div className="principleList"><div><strong>Work backward from arrival.</strong><span>The commitment anchors the morning.</span></div><div><strong>Do not rebuild Maps.</strong><span>Routing is infrastructure; the product owns the morning decision.</span></div><div><strong>Optimize for time kept.</strong><span>Arriving 30 minutes early is not automatically better.</span></div><div><strong>Model the miss.</strong><span>A six-minute train headway and a 30-minute bus headway are different risks.</span></div><div><strong>Freshness changes trust.</strong><span>A bus position updated 20 seconds ago should not be weighted like one last updated eight minutes ago.</span></div><div><strong>Personalize only where it earns accuracy.</strong><span>Health is optional and improves walking estimates; route-specific observed history becomes stronger evidence over time.</span></div><div><strong>Interrupt only when the plan changes.</strong><span>No alert for a delay that changes nothing.</span></div></div></section><section className="caseSection scopeLine"><h2>Scope</h2><div><p><strong>V1:</strong> calendar · routine · Health · wake/leave · BART/AC Transit · traffic · alarms</p><p><strong>Later:</strong> driving in Michigan · recurring commute detection · deeper reliability learning</p><p><strong>Cut:</strong> social features · generic trip planning · analytics dashboard · navigation replacement</p></div></section><section className="caseSection measureLine"><h2>What would tell me it works</h2><div><p><strong>Arrival error</strong> · predicted vs. observed &nbsp; <strong>Unused buffer</strong> · minutes returned &nbsp; <strong>Prediction error</strong> · routine/walk/route &nbsp; <strong>Interruptions</strong> · how often the app needs attention</p><p><strong>North star:</strong> minutes of unnecessary buffer returned while maintaining the user’s chosen on-time threshold.</p></div></section></>}
...
function Home({openCase}){
 ...
 <main id="main-content">
  <section id="top" className="hero v28Hero"><div className="heroInner"><h1>Neha Chinimilli</h1><p className="heroThesis">Computer Science + Supply Chain Management at Michigan State University</p><p className="heroTagline">I like building things that make life easier. Work smarter, not harder.</p><div className="heroLinks"><a className="primaryHeroLink" href="#projects">View projects ↓</a><a href="resume.pdf" target="_blank" rel="noreferrer">Resume ↗</a></div></div></section>
