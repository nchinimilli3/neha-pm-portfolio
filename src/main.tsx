...
function CommuteAppDemo(){
  ...
  const today=<div className="iosAppScreen todayScreen"><div className="appTop"><div><span>Friday, Aug 21</span><h3>{phase==='late'?'You need a new plan':`Good morning, ${name}`}</h3></div><button className="avatarBtn" type="button" aria-label="Open profile" onClick={()=>setDetail('profile')}>{name[0]||'N'}</button></div><div className="destinationLine"><div><span>Arrive by</span><strong>{destination}</strong></div><b>{fmt(arriveTarget)}</b></div>{phase==='late'?<div className="bigMoment urgent"><span>Leave now</span><strong>{fmt(chosenArrival)}</strong><small>Current arrival with {route}</small></div>:<div className="bigMoment"><span>{phase==='planned'?'Alarm':'Wake up'}</span><strong>{fmt(wakeMin)}</strong><small>Leave at {fmt(leaveMin)}</small></div>}<button className="routeRecommendation" type="button" onClick={()=>setDetail('why')}><div className="routeBadge">{takeBart?'B':'NL'}</div><span><small>Recommended</small><strong>{route}</strong><em>{routeSub}</em></span><div className="routeTimes"><b>{fmt(vehicleTime)}</b><small>arrive {fmt(chosenArrival)}</small></div></button><p className="routeReason">{routeReason}</p><div className="liveStrip"><span><Fresh source={takeBart?'bart':'nl'}/> {route}</span><span>{bridge==='clear'?'Bridge clear':bridge==='building'?'Bridge +6m':'Bridge +12m'}</span></div><button className="sleepAction" type="button" onClick={()=>setDetail('sleep')}><span>Can I sleep longer?</span><b>›</b></button></div>;
...
const toolLogoMap={
  react:['react','React','R'], mui:['mui','Material UI','MUI'], git:['git','Git','Git'], github:['github','GitHub Projects','GH'], typescript:['typescript','TypeScript','TS'], javascript:['javascript','JavaScript','JS'], html:['html5','HTML','HTML'], css:['css3','CSS','CSS'], figma:['figma','Figma','F'], kotlin:['kotlin','Kotlin','K'], spring:['spring','Spring','S'], excel:['microsoftexcel','Microsoft Excel','XL'], powerpoint:['microsoftpowerpoint','PowerPoint','PPT'], powerbi:['powerbi','Power BI','BI'], openai:['openai','OpenAI','AI'], aem:['adobeexperiencecloud','Adobe Experience Manager','AEM'], salesforce:['salesforce','Salesforce','SF'], postman:['postman','Postman','PM'], jenkins:['jenkins','Jenkins','J'], servicenow:['servicenow','ServiceNow','SN'], splunk:['splunk','Splunk','SP'], flask:['flask','Flask','F'], mysql:['mysql','MySQL','SQL'], socketio:['socketdotio','Socket.IO','IO'], docker:['docker','Docker','D'], gcp:['googlecloud','Google Cloud','GCP'], python:['python','Python','Py']
};
