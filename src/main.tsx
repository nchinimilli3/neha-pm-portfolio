...
const metrics = {
  kohler: [
    ['1 → any','SKU-to-market product vision','product'],
    ['8','audited workflow stages','system'],
    ['5+','regional document variables','scope'],
    ['3','enterprise source families','system']
  ],
  accenture: [
...
const ownership = {
  kohler:'On a five-person team, I helped shape the product and UX, then contributed across the React experience, Node services, Azure agent workflow, source integration, validation, and production delivery.',
...
const projects = [
  ...
  },
  {
    ...
    summary:'Built an AI export-compliance assistant that turns a SKU and destination into region-ready product documentation, guided preparation work, and an auditable record.',
    media:'kohler',
...
function AccentureCase(){return <div className="accentureStory">
  <CaseSection title="Context" className="accentureContext"><div className="accentureSplitCopy"><div><p>I joined Accenture's San Francisco team supporting go-to-market enablement for a frontier AI lab. Customer requests came in with different topics, regions, timelines, and delivery needs. Coordinators had to turn each request into a workable plan, find the right trainer, schedule delivery, collect feedback, and decide what should happen next.</p><p>My role sat across that operating workflow and the systems around it. I supported live requests first, then used what broke in practice to define what should be standardized, automated, or kept as a human decision.</p></div><blockquote>Support the work.<br/>See the pattern.<br/><em>Design the system.</em></blockquote></div></CaseSection>
  <CaseSection title="The problem" className="accentureProblem"><p>The process worked, but too much depended on coordinator judgment living in people's heads. Request details were inconsistent, trainer matching involved several constraints at once, and handoffs became harder as volume and global coverage grew.</p><div className="accentureTensions"><article><span>01</span><strong>Inconsistent intake</strong><p>Important request details arrived in different shapes.</p></article><article><span>02</span><strong>Multi-constraint matching</strong><p>Expertise alone could not produce a workable assignment.</p></article><article><span>03</span><strong>Fragile handoffs</strong><p>Global scale amplified every missing rule and assumption.</p></article></div></CaseSection>
  <CaseSection title="People in the system" className="accenturePeople"><div className="accenturePeopleFlow" aria-label="Stakeholder handoff from request to learning"><article><header><span>01</span><small>INITIATES</small></header><h3>Requester</h3><p>Needs the right enablement, with the right scope and timing.</p><b>REQUEST</b></article><i aria-hidden="true">→</i><article><header><span>02</span><small>TRANSLATES</small></header><h3>Coordinator</h3><p>Turns an ambiguous request into a deliverable plan.</p><b>PLAN</b></article><i aria-hidden="true">→</i><article><header><span>03</span><small>DELIVERS</small></header><h3>Trainer</h3><p>Needs an assignment that matches expertise, language, availability, and local working hours.</p><b>SESSION</b></article><i aria-hidden="true">→</i><article><header><span>04</span><small>EXPERIENCES</small></header><h3>Learner / Enterprise Customer</h3><p>Needs training that is useful after the session ends.</p><b>OUTCOME</b></article></div></CaseSection>
  <CaseSection title="How the workflow worked" className="accentureJourneySection"><div className="journeyFlow aiJourney"><div><i>01</i><strong>Request</strong><span>topic, region, audience, timing</span></div><b>→</b><div><i>02</i><strong>Match</strong><span>expertise, language, capacity, time zone</span></div><b>→</b><div><i>03</i><strong>Review</strong><span>warnings, reason codes, human check</span></div><b>→</b><div><i>04</i><strong>Deliver + learn</strong><span>session, feedback, follow-up</span></div></div></CaseSection>
  <CaseSection title="What became product requirements" className="accentureRequirements"><p>Supporting 21 live requests made the repeated decisions visible. I captured them in a 10-tab data contract covering required inputs, matching logic, validation rules, warnings, reason codes, and the points where a human still needed to review the recommendation.</p><AccentureWorkflowVisual/></CaseSection>
  <CaseSection title="The QA moment that changed the logic" className="accentureQa"><div className="edgeCaseCard"><span>EDGE CASE CAUGHT DURING QA</span><strong><mark>10:30 PM</mark> looked valid to the system.<br/>It was clearly wrong for the trainer.</strong><p>The match looked correct on expertise and availability, but it was not realistic. Working hours and time zones became explicit matching requirements instead of something a coordinator fixed after the fact.</p></div></CaseSection>
  <CaseSection title="Evidence behind the next phase" className="accentureEvidence"><div className="evidenceNumbers"><div><strong>~2,200</strong><span>learner responses synthesized</span></div><div><strong>~20 <i>→</i> 8</strong><span>providers researched → competitors compared</span></div><div><strong>27 <i>→</i> 12 <i>→</i> 5</strong><span>metrics → patterns → recommendations</span></div></div><p>I combined learner feedback with market and adoption research, then narrowed the findings into five recommendations and a 90-day path for what to test next.</p></CaseSection>
 </div>}

function KohlerCase(){
 const stages=[
  ['01','Order arrives','SKU + destination'],['02','Detect gap','Is regional documentation missing?'],['03','Retrieve','ERP + PIM + labelling'],['04','Compose','Market rules + translation'],['05','Validate','Completeness + source checks'],['06','Review','Human judgment when needed'],['07','Publish','Document packet saved'],['08','Continue','Audit record returns to order']
 ];
 return <div className="kohlerStory">
  <section className="kohlerProductStage"><KohlerProductSurface/><p className="kohlerPreviewCaption"><span>INTERACTIVE PRODUCT RECONSTRUCTION</span> Switch destinations to see the regional packet adapt. Sample data protects customer, order, and proprietary compliance information.</p></section>

  <CaseSection title="The order at risk" className="kohlerRiskSection"><div className="kohlerRiskLayout"><div className="kohlerOrderIllustration" aria-label="A stocked faucet is separated from export fulfillment by a missing regional document packet"><div className="kohlerSkuObject"><span>SKU K-14402</span><div className="kohlerFaucet" aria-hidden="true"><i></i><b></b><em></em></div><strong>Product ready</strong><small>Inventory confirmed</small></div><div className="kohlerMissingPacket"><span>DESTINATION LAYER</span><div><b>SPEC</b><b>WARRANTY</b><b>LABELS</b></div><strong>Documentation gap</strong><small>Required before the SKU can cross markets</small></div><div className="kohlerHeldOrder"><span>EXPORT ORDER · SO-28471</span><div className="kohlerRouteStamp"><b>US</b><i></i><b>IN</b></div><strong>Held before fulfillment</strong><small>Product exists. Market-ready evidence does not.</small></div><div className="kohlerOrderRail" aria-hidden="true"><i></i><em></em></div></div><p>When a requested SKU was not configured for its destination market, document readiness—not inventory—could stop the order. Ship Anywhere fills that exact gap: assemble the destination packet, validate it, preserve human control, and retain the evidence behind the decision.</p></div></CaseSection>

  <CaseSection title="People in the workflow" className="kohlerPeopleSection"><div className="kohlerPeopleMap"><div className="kohlerPeopleCore"><span>SHARED OBJECT</span><strong>One reviewable<br/>shipment packet</strong><small>Every role sees the same status and source trail.</small></div>{[
   ['Warehouse associate','Prepares the physical shipment','STARTS'],['Regional compliance','Reviews destination requirements','VERIFIES'],['Product data owner','Maintains approved attributes','SOURCES'],['Order operations','Moves the order forward','CLOSES']
  ].map(([role,job,verb],index)=><article className={`person-${index+1}`} key={role}><span>{String(index+1).padStart(2,'0')} · {verb}</span><strong>{role}</strong><p>{job}</p></article>)}</div></CaseSection>

  <CaseSection title="From order to auditable output" className="kohlerFlowSection"><div className="kohlerFlow"><div className="kohlerFlowLine" aria-hidden="true"><i></i></div>{stages.map(([number,title,copy],index)=><article key={number} style={{'--step':index}}><span>{number}</span><div><strong>{title}</strong><p>{copy}</p></div></article>)}</div></CaseSection>

  <CaseSection title="Source-of-truth architecture" className="kohlerArchitectureSection"><div className="kohlerArchitectureWrap"><div className="kohlerArchitecture" aria-label="Enterprise data moving through Azure orchestration into a reviewed document and audit record"><div className="kohlerSourceStack"><span>SOURCE SYSTEMS</span><article><img src={assetUrl('tool-logos/sap.svg')} alt=""/><div><strong>SAP ECC</strong><small>Order · SKU · destination</small></div></article><article><img src={assetUrl('tool-logos/inriver.webp')} alt=""/><div><strong>Inriver PIM</strong><small>Approved product attributes</small></div></article><article><b>LBL</b><div><strong>Labelling systems</strong><small>Market marks · packaging rules</small></div></article><p>Each fact keeps its source identity. The model cannot silently replace an approved system of record.</p></div><div className="kohlerDataCurrent"><span>GROUNDED CONTEXT</span><i></i><i></i><i></i></div><div className="kohlerAzureCore"><span>AZURE ORCHESTRATION</span><div className="kohlerCoreOrbit"><article><img src={assetUrl('tool-logos/azure-ai-foundry.svg')} alt=""/><strong>AI Foundry</strong></article><article><img src={assetUrl('tool-logos/copilot-studio.svg')} alt=""/><strong>Copilot Studio</strong></article><article><img src={assetUrl('tool-logos/databricks.svg')} alt=""/><strong>Databricks</strong></article><div><strong>Document<br/>decision layer</strong><small>retrieve · compose · check</small></div></div></div><div className="kohlerOutputBranch"><span>REVIEWABLE OUTPUTS</span><article><strong>Localized spec sheet</strong><small>Source-linked · versioned</small></article><article><strong>Guided work</strong><small>Checklist · exceptions · review gate</small></article><article><strong>Audit history</strong><small>Inputs · actor · timestamp · output</small></article><p>Nothing ships as a chat response. The final object is a governed packet a person can inspect.</p></div></div><div className="kohlerArchitectureLogic">{[['01','Fetch the order context','Only the SKU, destination, and order identity needed for this request.'],['02','Resolve authority','Product, market, and labelling facts retain source precedence and lineage.'],['03','Compose the regional delta','Stable product content stays fixed while local requirements are assembled.'],['04','Validate before release','Missing or conflicting evidence routes to a person instead of being guessed.']].map(([n,title,copy])=><article key={n}><span>{n}</span><strong>{title}</strong><small>{copy}</small></article>)}</div></div></CaseSection>

  <CaseSection title="How a regional specification is assembled" className="kohlerDocumentSection"><div className="kohlerDocumentAssembly"><div className="kohlerDocumentLayers"><article><span>STABLE CORE</span><strong>Product identity</strong><p>Approved name, dimensions, materials, features, imagery, and installation facts.</p></article><article><span>MARKET OVERLAY</span><strong>Destination rules</strong><p>Contacts, disclaimers, warranty, certifications, page format, and translated content.</p></article><article><span>ORDER CONTEXT</span><strong>Shipment record</strong><p>Order number, destination, requester, timestamp, and generated output version.</p></article></div><div className="kohlerSpecDiff"><header><span>US SOURCE</span><i>→</i><span>INDIA OUTPUT</span></header><div><small>PAGE FORMAT</small><b>Letter</b><strong>A4</strong></div><div><small>LANGUAGE</small><b>English</b><strong>English + Hindi</strong></div><div><small>WARRANTY</small><b>US market</b><strong>India market</strong></div><div><small>CONTACT</small><b>US support</b><strong>Regional support</strong></div><footer>Illustrative structure · not a real compliance packet</footer></div></div></CaseSection>

  <CaseSection title="What the agent decides—and what it does not" className="kohlerBoundarySection"><div className="kohlerBoundary"><article><span>01 · DETERMINISTIC</span><strong>Rules stay rules.</strong><p>Destination, page size, required fields, approved source precedence, and completion status are checked explicitly.</p><small>SYSTEM ENFORCES</small></article><article><span>02 · MODEL-ASSISTED</span><strong>AI handles synthesis.</strong><p>The agent retrieves context, drafts localized content, explains missing inputs, and assembles a reviewable packet.</p><small>MODEL PROPOSES</small></article><article><span>03 · HUMAN-CONTROLLED</span><strong>Ambiguity escalates.</strong><p>Conflicting data, unsupported claims, translation uncertainty, and incomplete evidence stop before release.</p><small>PERSON APPROVES</small></article></div></CaseSection>

  <CaseSection title="Exceptions designed into the happy path" className="kohlerExceptionSection"><div className="kohlerExceptionLedger">{[
   ['Missing product attribute','Route to the product-data owner','No unsupported value is generated.'],['Conflicting regional source','Show both sources and precedence','A reviewer resolves the conflict.'],['Translation needs review','Flag the affected passage','The rest of the packet can continue.'],['Compliance evidence incomplete','Block document approval','The audit trail records what is missing.']
  ].map(([signal,response,guardrail],index)=><article key={signal}><span>{String(index+1).padStart(2,'0')}</span><strong>{signal}</strong><p>{response}</p><small>{guardrail}</small></article>)}</div></CaseSection>

  <CaseSection title="The record behind every document" className="kohlerAuditSection"><div className="kohlerAudit"><header><span>AUDIT · SO-28471</span><strong>Traceable from request to release</strong></header><ol><li><time>09:12</time><i></i><div><strong>Order context received</strong><small>User, order, SKU, and destination captured.</small></div></li><li><time>09:14</time><i></i><div><strong>Authoritative sources retrieved</strong><small>Product and regional records versioned.</small></div></li><li><time>09:17</time><i></i><div><strong>Localized packet generated</strong><small>Inputs and generated output retained together.</small></div></li><li><time>09:24</time><i></i><div><strong>Human review completed</strong><small>Reviewer, timestamp, and final document recorded.</small></div></li></ol><footer><span>WHO</span><span>WHAT</span><span>WHEN</span><span>OUTPUT</span></footer></div></CaseSection>

  <CaseSection title="Production delivery" className="kohlerDeliverySection"><div className="kohlerDelivery"><p>We delivered the production workflow as a five-person capstone team. My contribution crossed product definition, interaction design, React and Node implementation, Azure agent orchestration, enterprise-source integration, validation, and release work.</p><div className="kohlerShippedList"><article><span>SHIPPED</span><strong>Region-specific document generation</strong></article><article><span>SHIPPED</span><strong>Missing-information and exception handling</strong></article><article><span>SHIPPED</span><strong>Human review and auditable history</strong></article></div><div className="kohlerReleaseMoment"><div><span>ORDER SO-28471</span><strong>Released for export</strong><small>Validated packet attached · audit record complete</small></div><div className="kohlerFinalRoute" aria-hidden="true"><b>US</b><i><em></em></i><b>IN</b></div><p><span>BEFORE</span><s>Documentation blocks the order.</s><span>AFTER</span><strong>An associate reviews the packet, approves it, and the order continues.</strong></p></div><blockquote>The happy ending is intentionally quiet: the product exists, the evidence is trustworthy, and the shipment can move.</blockquote></div></CaseSection>
 </div>
}
