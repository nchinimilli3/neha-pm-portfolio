...
function PhotoCollectionCarousel({section,onExpand}){
  ...
  return <section id={section.id} className="photoCollectionSection photoCarouselSection"><div className="photoCollectionMeta"><p>{section.note}</p><h2>{section.title}</h2><span>{section.description}</span></div><div className={`photoCarousel photo-${orientation}`} onTouchStart={e=>{touchStart.current=e.touches[0]?.clientX??null}} onTouchEnd={e=>{if(touchStart.current===null)return;const x=e.changedTouches[0]?.clientX??touchStart.current;const delta=x-touchStart.current;if(Math.abs(delta)>45)move(delta<0?1:-1);touchStart.current=null}}><button type="button" className={`photoCarouselStage ${orientation}`} onClick={()=>onExpand({...activePhoto,src:activeSrc})} aria-label={`Open ${section.title} photo ${index+1}`}><img key={activePhoto.src} onLoad={setImageOrientation} src={activeSrc} alt={activePhoto.alt}/></button><div className="photoCarouselControls"><div><button type="button" onClick={()=>move(-1)} aria-label={`Previous ${section.title} photo`}>←</button><button type="button" onClick={()=>move(1)} aria-label={`Next ${section.title} photo`}>→</button></div><span>{String(index+1).padStart(2,'0')} / {String(count).padStart(2,'0')}</span></div><div className="photoFilmstrip" aria-label={`${section.title} thumbnails`}>{section.photos.map((photo,i)=><button type="button" className={i===index?'active':''} key={photo.src} onClick={()=>setIndex(i)} aria-label={`Show ${section.title} photo ${i+1}`} aria-current={i===index?'true':undefined}><img decoding="async" src={photoAsset(photo.src)} alt=""/></button>)}</div></div></section>
}
...
const experienceItems=[
 ...
 {id:'pwc',company:'PwC × Arc of Indiana',role:'Consulting Extern',location:'',dates:'Aug-Oct 2024',logo:'company-logos/pwc-v31.png',short:'Turned peer benchmarking into a repeatable strategy framework for a nonprofit client.',detail:<div className="expStory"><p>Over a roughly 10-week externship, I independently researched The Arc of Indiana, a statewide nonprofit serving people with intellectual and developmental disabilities and their families, and the organizations it could learn from. I defined the comparison criteria myself, built a weighted seven-category scorecard, benchmarked five peer organizations across 10+ engagement and innovation metrics, and translated the research into recommendations for the client.</p><div className="expMetricRow"><span><b>7</b> scorecard categories</span><span><b>5</b> peer organizations</span><span><b>10+</b> metrics</span><span><b>5</b> recommendations adopted</span></div></div>},
...
function Home({openCase}){
 ...
 <main id="main-content">
  ...
  <ExperienceSection onAura={setAuraTone}/><EducationSection/>
  <section id="fun" className="section moreSection v28Fun"><div className="sectionTitle compactTitle"><h2>Fun things I’ve built</h2></div><div className="funLeadGrid">{fun.map(p=><MoreProjectCard key={p.id} project={p} onOpen={openCase}/>)}</div><div className="smallBuildGrid"><article className="smallBuild"><div className="techVisual game"><div className="spartanScene"><img className="spartanBg" src="project-media/spartan-background.png" alt="Spartan Touchdown level"/><div className="spartanGround"></div><img className="spartySprite" src="project-media/sparty.png" alt="Sparty"/><img className="enemySprite" src="project-media/um-enemy.png" alt="Michigan enemy"/></div></div><h3>Spartan Touchdown</h3><p>Team-built C++ football game with player movement, collisions, enemies, scoring, and level logic.</p></article><article className="smallBuild"><div className="techVisual fluids"><img src="project-media/stable-fluids.png" alt="Stable Fluids simulation"/></div><h3>Stable Fluids</h3><p>Interactive C++ fluid simulation modeling density and velocity fields with user-driven forces and real-time rendering.</p></article></div></section>
  <section id="about" className="section aboutSection"><div className="aboutPhoto"><img src="headshot.jpg" alt="Neha Chinimilli" width={1066} height={1599}/></div><div className="aboutCopy"><h2>About me</h2><p>I’m Neha. I study <strong>Computer Science and Supply Chain Management at Michigan State</strong>, which means I spend a lot of time thinking about how systems work and how people actually use them.</p><p>Case in point: since high school, I’ve been programming some version of a tool to help me wake up on time and check my commute. It started as a simple script. Now it’s an iOS app that works backward from where I need to be, my morning routine, and live transit conditions so I do not have to redo the same calculation every morning.</p><p className="hobbyLine">Outside of work, I’m into coffee, traveling, <HobbyPopover label="reading" title="On my shelf" items={["A Thousand Splendid Suns","When Breath Becomes Air","The Year of Magical Thinking","Sharp Objects"]}/>, <HobbyPopover label="reality TV" title="Always on rotation" items={["Modern Family","Vanderpump Rules","Summer House","the newest Real Housewives season"]}/>, baking, hiking, painting, and film photography.</p><div className="aboutActions"><a className="photoCta" href="#/photography">Check out my pics →</a><BookRecForm/></div></div></section>
