...
const toolLogoMap={
  ...
  cloudflarepages:{name:'Cloudflare Pages',mark:'Pages',urls:[],textOnly:true},
  ...
  googlebooks:{name:'Google Books',mark:'Books',urls:[],textOnly:true},
  openlibrary:{name:'Open Library',mark:'OL',urls:[],textOnly:true},
...
function BookclubCase(){return <>
  ...
  <CaseSection title="The product loop"><div className="bookclubLoop">{[['01','Create or join','Invite-only clubs'],['02','Nominate','Build a focused shortlist'],['03','Rank','Capture real preferences'],['04','Read','Move at a flexible pace'],['05','Discuss','Protect spoiler boundaries'],['06','Meet','Poll, RSVP, and add to calendar'],['07','Rate','Close the active read'],['08','Repeat','Start again from the shared shelf']].map(([number,title,copy])=><div key={number}><span>{number}</span><strong>{title}</strong><p>{copy}</p></div>)}</div></CaseSection>
  <CaseSection title="Product decisions"><div className="bookclubDecisionGrid"><div><strong>Private by default</strong><span>Clubs are invite-only spaces for an existing group, not public feeds built around performance.</span></div><div><strong>Preference voting</strong><span>Ranked choices and runoff handling reveal acceptable group options instead of rewarding the loudest favorite.</span></div><div><strong>Flexible pacing</strong><span>Page or chapter progress lets each reader move honestly without turning the plan into social pressure.</span></div><div><strong>Spoiler boundaries</strong><span>Checkpoints anchor discussion to reading progress so a useful post does not expose what comes next.</span></div><div><strong>Private or shared notes</strong><span>Quotes, questions, and predictions can stay personal or become part of the club conversation.</span></div><div><strong>Lightweight coordination</strong><span>A few realistic meeting options, RSVPs, reminders, and calendar handoff keep planning proportional to the group.</span></div></div></CaseSection>
  <CaseSection title="Product walkthrough"><div className="bookclubWalkthrough"><article><figure><img loading="lazy" decoding="async" src={assetUrl('project-media/bookclub/selection-live.jpg')} alt="Live Bookclub nomination screen showing the private first-pick workflow"/><figcaption>Live product · nomination and first-pick workflow</figcaption></figure><div><span>Choose together</span><h3>Make the next-read decision visible.</h3><p>Members search across book sources, nominate a small set, rank preferences, and resolve ties or runoffs inside the club instead of rebuilding the decision in a group chat.</p></div></article><article><figure className="bookclubWalkthroughWide"><img loading="lazy" decoding="async" src={assetUrl('project-media/bookclub/current-reading-live.jpg')} alt="Live Bookclub club home showing the current book, progress, next checkpoint, and meeting state"/><figcaption>Live product · active club home</figcaption></figure><div><span>Orient the group</span><h3>Put the current reading state in one place.</h3><p>The club home connects the active book, individual progress, next checkpoint, Reading Room, and upcoming meeting so every member can answer “what now?” quickly.</p></div></article><article><figure><img loading="lazy" decoding="async" src={assetUrl('project-media/bookclub/reading-room-live.jpg')} alt="Live Bookclub Reading Room showing current page progress and reading-plan navigation"/><figcaption>Live product · Reading Room and progress</figcaption></figure><div><span>Read at an honest pace</span><h3>Keep everyone in the same book without forcing the same speed.</h3><p>Members update by page or chapter. A reading plan turns the finish date into checkpoints while the interface keeps each person’s place visible.</p></div></article><article><figure className="bookclubWalkthroughCheckpoint"><img loading="lazy" decoding="async" src={assetUrl('project-media/bookclub/checkpoints-live.jpg')} alt="Live Bookclub reading plan showing progress checkpoints and calendar actions"/><figcaption>Live product · checkpoints and calendar handoff</figcaption></figure><div><span>Protect the conversation</span><h3>Anchor discussion to progress.</h3><p>Thoughts, questions, predictions, quotes, replies, reactions, and agenda items live beside checkpoints, giving the group a spoiler-aware place to remember what mattered before the meeting.</p></div></article><article><figure><img loading="lazy" decoding="async" src={assetUrl('project-media/bookclub-preview.png')} alt="Bookclub finished-books shelf showing completed reads"/><figcaption>Live product · finished-books shelf</figcaption></figure><div><span>Close the loop</span><h3>Turn one finished book into the next cycle.</h3><p>Meeting polls, RSVPs, reminders, ratings, and the finished shelf preserve the group’s history and create a clear return path to the next shortlist.</p></div></article></div></CaseSection>
  ...
  <CaseSection title="System design"><p>React, TypeScript, and Vite power the product interface on Cloudflare Pages. Cloudflare Workers handle server-side integrations, OCR and AI workflows, calendar actions, and scheduled reminders. D1 stores relational club data; R2 stores product media.</p><div className="bookclubArchitecture" aria-label="Bookclub system architecture"><div><span>Product interface</span><strong>React · TypeScript · Vite</strong><small>Cloudflare Pages</small></div><b>→</b><div><span>Server layer</span><strong>Cloudflare Workers</strong><small>integrations · workflows · reminders</small></div><b>→</b><div><span>Data</span><strong>D1 · R2</strong><small>relational data · media</small></div></div><p className="diagramNote">Connected services: Google Calendar, NYT Books, Google Books, Open Library, OpenAI, and Google Vision OCR.</p></CaseSection>
  <CaseSection title="What I’m measuring next"><p className="bookclubMeasureIntro">The product is live; I am not presenting adoption or outcome claims before the group has generated enough evidence. The next useful measures are behavioral:</p><div className="bookclubMeasures"><div><strong>Selection momentum</strong><span>Time from first nomination to a completed choice, including where a vote stalls.</span></div><div><strong>Reading participation</strong><span>Checkpoint completion and progress updates while a book is active.</span></div><div><strong>Discussion usefulness</strong><span>Questions, quotes, and predictions that become replies or meeting agenda items.</span></div><div><strong>Club continuity</strong><span>Finished reads that progress into ratings and a new shortlist.</span></div></div></CaseSection>
</>}
