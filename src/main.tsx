...
const projects = [
  ...
  },
  {
    ...
    summary:'Translated live enablement friction into a six-stage operating model, testable automation requirements, and evidence-backed recommendations for the next phase.',
    media:'accenture',
  ...
  },
  {
    ...
    title:'FinSimple',
    company:'Ford Credit',
    summary:'Owned the Previous Estimates feature inside an existing financial platform, taking it from requirements through AEM component work, Salesforce/API integration, QA, stakeholder coordination, and production validation.',
    media:'finsimple',
...
const photoCollections=[
  ...
  },
  {
    ...
    title:'MSU grad',
    note:'East Lansing',
    description:'ColorPlus 200 & Portra 400',
    photos:[
  ...
  },
  {
    ...
    title:'San Francisco',
    note:'California',
    description:'Ultramax 400 and Portra 400',
    photos:[
      ...
      {src:'photography/gallery/sf/sf-4.jpg',alt:'Painted Ladies with the San Francisco skyline in clear afternoon light'},
      {src:'photography/gallery/sf/sf-5.jpg',alt:'Golden Gate Bridge in hazy evening light above flowering plants'},
      {src:'photography/gallery/sf/sf-6.jpg',alt:'Detailed Victorian homes in warm San Francisco light'}
    ]
  },
  {
    id:'lake-tahoe',
  ...
  },
  {
    ...
    title:'Chicago',
    note:'Illinois',
    description:'Ultramax 400',
    photos:[
      {src:'photography/gallery/chicago/chicago-1.jpg',alt:'Art Deco high-rise framed between downtown Chicago buildings'},
...
const photoAsset=assetUrl;
function PhotoCollectionCarousel({section,onExpand}){
  const [index,setIndex]=useState(0);
  const [orientation,setOrientation]=useState('landscape');
  const touchStart=useRef(null);
  const count=section.photos.length;
  const move=delta=>setIndex(i=>(i+delta+count)%count);
  useEffect(()=>{setIndex(0);setOrientation('landscape')},[section.id]);
  const setImageOrientation=e=>{const {naturalWidth:w,naturalHeight:h}=e.currentTarget;setOrientation(h>w*1.12?'portrait':w>h*1.12?'landscape':'square')};
  const activePhoto=section.photos[index];
  const activeSrc=photoAsset(activePhoto.src);
  return <section id={section.id} className="photoCollectionSection photoCarouselSection"><div className="photoCollectionMeta"><p>{section.note}</p><h2>{section.title}</h2><span>{section.description}</span></div><div className={`photoCarousel photo-${orientation}`} onTouchStart={e=>{touchStart.current=e.touches[0]?.clientX??null}} onTouchEnd={e=>{if(touchStart.current===null)return;const x=e.changedTouches[0]?.clientX??touchStart.current;const delta=x-touchStart.current;if(Math.abs(delta)>45)move(delta<0?1:-1);touchStart.current=null}}><button type="button" className={`photoCarouselStage ${orientation}`} onClick={()=>onExpand({...activePhoto,src:activeSrc})} aria-label={`Open ${section.title} photo ${index+1}`}><img key={activePhoto.src} loading="eager" decoding="async" onLoad={setImageOrientation} src={activeSrc} alt={activePhoto.alt}/></button><div className="photoCarouselControls"><div><button type="button" onClick={()=>move(-1)} aria-label={`Previous ${section.title} photo`}>←</button><button type="button" onClick={()=>move(1)} aria-label={`Next ${section.title} photo`}>→</button></div><span>{String(index+1).padStart(2,'0')} / {String(count).padStart(2,'0')}</span></div><div className="photoFilmstrip" aria-label={`${section.title} thumbnails`}>{section.photos.map((photo,i)=><button type="button" className={i===index?'active':''} key={photo.src} onClick={()=>setIndex(i)} aria-label={`Show ${section.title} photo ${i+1}`} aria-current={i===index?'true':undefined}><img loading="eager" decoding="async" src={photoAsset(photo.src)} alt=""/></button>)}</div></div></section>
}
function PhotographyPage({onBack}){
  const [lightbox,setLightbox]=useState(null);
  const featured=photoCollections.find(section=>section.id==='san-francisco')?.photos?.[3]||photoCollections[0].photos[0];
  useLayoutEffect(()=>{const root=document.documentElement;const previous=root.style.scrollBehavior;root.style.scrollBehavior='auto';window.scrollTo(0,0);root.style.scrollBehavior=previous},[]);
  const jumpTo=id=>document.getElementById(id)?.scrollIntoView({behavior:'smooth',block:'start'});
  return <main className="photoPage photoGalleryPage"><AuraField tone="photography"/><header className="photoPageHeader"><button type="button" onClick={onBack}>← Portfolio</button><span>Photography</span></header><section className="photoGalleryIntro"><div className="photoGalleryCopy"><p>35mm film</p><h1>Portfolio</h1></div><figure className="photoHeroShot"><img loading="eager" decoding="async" src={photoAsset(featured.src)} alt={featured.alt}/></figure></section><nav className="photoCollectionNav" aria-label="Photography collections">{photoCollections.map(section=><button type="button" key={section.id} onClick={()=>jumpTo(section.id)}>{section.title}</button>)}</nav>{photoCollections.map(section=><PhotoCollectionCarousel key={section.id} section={section} onExpand={setLightbox}/>)}<ImageLightbox image={lightbox} onClose={()=>setLightbox(null)}/></main>
}
...
function ToolLogoStrip({id}){const items=toolSets[id]||[];if(!items.length)return null;return <section className="toolLogoSection"><div className="toolLogoHeading"><h2>Tech stack</h2><p>{id==='fcvf'?'Tools and technologies used across the FCVF build.':id==='finsimple'?'Tools and platform technologies used in the FinSimple work.':'Tools used in this project.'}</p></div><div className="toolLogoRow">{items.map(k=><ToolLogo key={k} tool={toolLogoMap[k]}/>)}</div></section>}
...
const experienceItems=[
 ...
 {id:'pwc',company:'PwC × Arc of Indiana',role:'Consulting Extern',location:'',dates:'Aug-Oct 2024',logo:'company-logos/pwc-v31.png',short:'Turned peer benchmarking into a repeatable strategy framework for a nonprofit client.',detail:<div className="expStory"><p>Over a five-week externship, I independently researched The Arc of Indiana, a statewide nonprofit serving people with intellectual and developmental disabilities and their families, and the organizations it could learn from. I defined the comparison criteria myself, built a weighted seven-category scorecard, benchmarked five peer organizations across 10+ engagement and innovation metrics, and translated the research into recommendations for the client.</p><div className="expMetricRow"><span><b>7</b> scorecard categories</span><span><b>5</b> peer organizations</span><span><b>10+</b> metrics</span><span><b>5</b> recommendations adopted</span></div></div>},
...
function Home({openCase}){
 ...
 const serious=['fcvf','accenture','finsimple','scheduler'].map(id=>projects.find(p=>p.id===id)).filter(Boolean);
...
function App(){
 ...
 if(current.type==='photography')return <><PhotographyPage onBack={closeRoute}/><ScrollToTopButton/></>;
