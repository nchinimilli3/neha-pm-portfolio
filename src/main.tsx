...
function FCVFCase({setLightbox}){
 ...
 return <div className="fcvfStory">
  <section className="fcvfFeatureStage" aria-label="Interactive illustration of the final multi-page assessment"><div className="fcvfStageCopy"><span>Multi-page assessment</span><h2>One question at a time.</h2><p>The framework still calculates customer value. The experience now protects attention—and the integrity of the answers used to reach that score.</p><div className="fcvfStageSignals"><i>Focused step</i><i>Visible progress</i><i>Score revealed after</i></div></div><div className="fcvfStageWindow"><div className="fcvfWindowBar"><span></span><span></span><span></span><b>Customer Value Framework</b></div><div className="fcvfWindowProgress"><i></i><i></i><i className="active"></i><i></i><i></i><i></i></div><span className="fcvfQuestionLabel">QUESTION 3 OF 6</span><h3>How clearly does this product solve a customer problem?</h3><div className="fcvfChoices"><span>Not yet</span><span>Partly</span><span className="selected">Clearly</span></div><div className="fcvfWindowFoot"><span>Progress is visible. Score is not.</span><b>Next →</b></div></div></section>
  ...
  <CaseSection title="Research changed the product" className="fcvfResearchSection"><div><p className="fcvfSectionLead">I interviewed four users and compared one-page and multi-page directions. The key finding was not about polish; it changed what the interface should reveal, and when.</p><div className="fcvfResearchPath"><article><b>1</b><span>What users felt</span><strong>“Seeing the full assessment at once feels overwhelming.”</strong></article><i>→</i><article><b>2</b><span>What I identified</span><strong>A live score invites people to revisit answers and push the result.</strong></article><i>→</i><article className="decision"><b>3</b><span>What changed</span><strong>Paginate the flow. Show progress now; reveal the score after.</strong></article></div><div className="fcvfBiasMechanic"><div className="fcvfLiveScore"><header><span>Score while answering</span><s>VISIBLE</s></header><strong>62</strong><div><i style={{'--score':'62%'}}></i></div><small>Changing an answer changes the target.</small></div><div className="fcvfBiasArrow" aria-hidden="true">→</div><div className="fcvfProgressOnly"><header><span>Progress while answering</span><b>PROTECTED</b></header><div>{[0,1,2,3,4,5].map(index=><i className={index<3?'done':''} key={index}></i>)}</div><strong>3 of 6</strong><small>The respondent focuses on the question, not the score.</small></div></div></div></CaseSection>
