...
const ownership = {
  ...
  commute:'I framed the product around one recurring decision: when a morning needs to start. I scoped the first version to a commute I knew well, designed the recommendation logic around uncertainty instead of raw ETA, and built the portfolio sandbox to make those trade-offs tangible.',
...
const projects = [
  {
    ...
    summary:'Works backward from where you need to be to tell you when to wake up, when to leave, and which familiar route is worth taking that morning.',
    media:'commute',
    facts:['Interactive product sandbox','Morning timing · transit reliability']
  },
  {
    id:'fcvf',
...
function CommutePhone({view='home',busDelay=6,bridge='building',bartDelay=0,routine=48,health=true,wait=0,stale=false,onWait}){
  ...
  const routeLeave=takeBart?8*60+6:8*60+9;
  const conditionAdjustment=takeBart?Math.ceil(bartDelay*.7)+(stale?2:0):Math.ceil(busDelay*.45)+(bridge==='heavy'?3:bridge==='building'?1:0)+(stale?2:0);
  const leave=routeLeave-conditionAdjustment;
  const effectiveRoutine=Math.max(18,routine-wait);
  const wake=leave-effectiveRoutine;
  if(view==='why') return <div className="commutePhone"><div className="phoneStatus">9:41</div><div className="commuteNav">← <strong>Why {recommendation}?</strong></div><div className="whyLead">{takeBart?'The bus could still be faster. But missing it would make you late.':'BART is steadier, but the bus is worth it right now.'}</div><p className="whyCopy">{takeBart?`The NL is running about ${busDelay} min behind and bridge conditions are ${bridge}. BART gives you more room if your first departure slips.`:`The bus is close, bridge conditions are ${bridge}, and the next BART adds more waiting than it saves.`}</p><div className="routeCompare"><div><strong>BART</strong><span>Leave {fm(leave)}</span><span>Arrive 8:48–8:53</span><small>Miss it? Next train ~6 min later</small></div><div><strong>NL</strong><span>Leave 8:09</span><span>Arrive 8:42–9:04</span><small>Miss it? Next useful departure ~28 min later</small></div></div><div className="dataFoot">Demo logic · live-feed states are simulated in this portfolio sandbox.</div></div>;
  if(view==='routine') return <div className="commutePhone"><div className="phoneStatus">9:41</div><div className="commuteNav">← <strong>Learn your morning</strong></div><p className="routineIntro">What usually happens before you leave?</p><div className="routineRows"><div><span>Shower</span><strong>10 min</strong></div><div><span>Get ready</span><strong>25 min</strong></div><div><span>Breakfast</span><strong>8 min</strong></div><div><span>Out the door</span><strong>5 min</strong></div></div><div className="routineTotal"><span>Your usual morning</span><strong>~{routine} min</strong></div><div className="healthRow"><span>Walking personalization</span><strong>{health?'Health connected':'Standard estimate'}</strong></div><p className="tinyCopy">The app starts with your estimate, then asks before adopting changes it learns from actual mornings.</p></div>;
  return <div className="commutePhone"><div className="phoneStatus">9:41</div><div className="commuteDate">FRIDAY · AUG 21</div><div className="commuteDestination"><span>Salesforce Tower</span><strong>9:00 AM</strong></div><div className="wakeHero"><strong>{fm(wake)}</strong><span>Wake up</span><small>Alarm set</small></div><div className="morningLine"><div><b>{fm(leave)}</b><span>Leave home</span></div><div><b>8:19</b><span>19th St BART</span></div><div><b>8:51</b><span>Salesforce Tower</span></div></div><div className="recommendLine"><strong>{recommendation}</strong><span>{takeBart?'19th St → Embarcadero':'Grand Ave → Salesforce Transit Center'}</span><small>{stale?'Live feed stale · using schedule + history':takeBart?'BART is the safer choice this morning.':'The bus is worth catching this morning.'}</small></div><button className="sleepBtn" onClick={onWait}>Can I sleep longer?</button></div>
}
...
function CommuteSandbox(){
  ...
  const [routine,setRoutine]=useState(48);
  const [health,setHealth]=useState(true);
  const [wait,setWait]=useState(0);
  const [stale,setStale]=useState(false);
  const [toast,setToast]=useState('');
  const waitAction=()=>{setWait(w=>Math.min(15,w+5));setToast(wait>=10?'That would compress the routine too far.':'Still workable — the wake time moved 5 min later.');setTimeout(()=>setToast(''),1800)};
  return <div className="commuteSandbox">
    <div className="sandboxTopline"><div><span>Portfolio sandbox</span><strong>Try the morning decision engine</strong></div><small>Simulated live conditions · no claims of production accuracy</small></div>
    <div className="commuteSandboxGrid">
      <aside className="commuteControls"><h3>Change the morning</h3><div className="scenarioRow"><button onClick={()=>{setBusDelay(2);setBridge('clear');setBartDelay(0);setStale(false)}}>Normal</button><button onClick={()=>{setBusDelay(12);setBridge('heavy');setBartDelay(0);setStale(false)}}>Bridge mess</button><button onClick={()=>{setBusDelay(0);setBridge('clear');setBartDelay(9);setStale(false)}}>BART delay</button><button onClick={()=>setStale(true)}>Stale feed</button></div><label>Bus delay <b>+{busDelay} min</b><input type="range" min="0" max="18" value={busDelay} onChange={e=>setBusDelay(+e.target.value)}/></label><label>BART delay <b>+{bartDelay} min</b><input type="range" min="0" max="15" value={bartDelay} onChange={e=>setBartDelay(+e.target.value)}/></label><label>Bridge conditions<select value={bridge} onChange={e=>setBridge(e.target.value)}><option value="clear">Clear</option><option value="building">Building</option><option value="heavy">Heavy</option></select></label><label>Morning routine <b>{routine} min</b><input type="range" min="20" max="75" value={routine} onChange={e=>setRoutine(+e.target.value)}/></label><label className="checkLabel"><input type="checkbox" checked={health} onChange={e=>setHealth(e.target.checked)}/> Use personalized walking estimate</label><label className="checkLabel"><input type="checkbox" checked={stale} onChange={e=>setStale(e.target.checked)}/> Simulate stale transit feed</label><button onClick={()=>{setBusDelay(6);setBridge('building');setBartDelay(0);setRoutine(48);setHealth(true);setWait(0);setStale(false)}}>Reset morning</button></aside>
      <div className="sandboxDevice"><div className="screenTabs"><button className={screen==='home'?'active':''} onClick={()=>setScreen('home')}>Today</button><button className={screen==='why'?'active':''} onClick={()=>setScreen('why')}>Why this route</button><button className={screen==='routine'?'active':''} onClick={()=>setScreen('routine')}>Routine</button></div><CommutePhone view={screen} busDelay={busDelay} bridge={bridge} bartDelay={bartDelay} routine={routine} health={health} wait={wait} stale={stale} onWait={waitAction}/>{toast&&<div className="sandboxToast">{toast}</div>}</div>
      <aside className="decisionReadout"><span>What the app is optimizing</span><h3>Latest safe start, not shortest ETA.</h3><p>It weighs the cost of missing an infrequent bus, current service conditions, walking time, and the time the user actually needs before leaving.</p><div className="decisionRule"><strong>{busDelay + (bridge==='heavy'?10:bridge==='building'?5:0) > bartDelay+4?'BART wins this state':'Bus wins this state'}</strong><span>The recommendation changes when the inputs change.</span></div><div className="mundaneState"><b>Data quality</b><span>{stale?'Live feed is stale. The UI lowers confidence and falls back instead of pretending the ETA is exact.':'Live feeds current in this simulated state.'}</span></div></aside>
    </div>
  </div>
}

function CommuteInUse(){return <div className="commuteInUse"><figure className="usagePhoto"><img src="https://images.unsplash.com/photo-1671641911931-c8b5f05f4251?fm=jpg&q=76&w=1600" alt="Salesforce Tower in San Francisco, photo by Georg Eiermann on Unsplash"/><figcaption>Real destination context · Salesforce Tower, San Francisco</figcaption></figure><div className="usagePhone"><CommutePhone busDelay={2} bridge="clear"/></div><div className="lockMock"><span>7:54</span><strong>Leave in 12 min</strong><small>BART · 19th St</small><i>Everything is on track.</i></div></div>}
...
function CaseStudy({id,onBack}){
 ...
 {id==='commute'&&<><section className="sandboxSection commuteShowcase"><CommuteSandbox/></section><CaseSection title="The observation"><p>During a summer commute from Oakland to Salesforce Tower, I rarely needed help discovering a route. I already knew the familiar choices. The repeated decision was smaller and more annoying: how late could I stay home, and which option was worth trusting that morning?</p></CaseSection><CaseSection title="Product opinion"><div className="commuteOpinion"><strong>The home screen answers one question first.</strong><span>When does my morning need to start?</span></div><p>I deliberately did not make the product another map-first route planner. Routing is infrastructure. The product layer works backward from an arrival commitment and turns changing conditions into a wake time, leave time, and one recommended familiar route.</p></CaseSection><CaseSection title="Product principles"><div className="principleList"><div><strong>Decide once, then get out of the way.</strong><span>Recurring trips should not require a fresh route-planning ritual every morning.</span></div><div><strong>Uncertainty is part of the product.</strong><span>A wide arrival window or stale feed is more useful than a precise-looking number the system cannot defend.</span></div><div><strong>Personalization must earn trust.</strong><span>Health, Calendar, and observed behavior reduce setup over time, but the user stays in control of what the app learns and changes.</span></div><div><strong>Interrupt only when the plan changes.</strong><span>Traffic and service alerts are inputs, not content. They deserve attention only when the user should wake, leave, or travel differently.</span></div></div></CaseSection><CaseSection title="How the decision works"><div className="decisionSteps"><div><span>1</span><strong>Start with the commitment</strong><p>Calendar location, arrival time, and the user's preferred buffer.</p></div><div><span>2</span><strong>Model the real morning</strong><p>Routine duration, door-to-platform time, walking personalization, and whether the user can safely wait.</p></div><div><span>3</span><strong>Compare uncertainty</strong><p>Live transit state, traffic exposure, missed-departure penalty, service alerts, and stale-data fallbacks.</p></div><div><span>4</span><strong>Return one decision</strong><p>Wake at X. Leave at Y. Take this route today. Explain the trade-off only when asked.</p></div></div></CaseSection><CaseSection title="What I chose not to build"><div className="productCuts"><div><strong>No map on Home</strong><span>The user already knows the commute; a map competes with the timing decision.</span></div><div><strong>No mystery reliability score</strong><span>The prototype avoids fake precision. Real percentages only belong once the model is calibrated against observed trips.</span></div><div><strong>No dashboard of conditions</strong><span>Weather, traffic, and alerts surface only when they change what the user should do.</span></div><div><strong>No forced Health or Calendar setup</strong><span>Both improve automation, but the core flow still works with manual inputs.</span></div></div></CaseSection><CaseSection title="Learn the morning"><p>Onboarding asks for a rough morning sequence once, then the product learns carefully. It can compare the user's estimate with observed mornings, but it asks before changing the routine it uses. The same principle applies to walking pace: Health can provide an optional starting estimate, while route-specific history becomes more useful over time.</p><div className="morningModes"><div><strong>Normal</strong><span>Full routine · 48 min</span></div><div><strong>Quick</strong><span>Compressed routine · 28 min</span></div><div><strong>Bare minimum</strong><span>Only essentials · 14 min</span></div></div></CaseSection><CaseSection title="Do less over time"><div className="automationFlow"><div><strong>Calendar</strong><span>Find the first place the user physically needs to be.</span></div><b>→</b><div><strong>Morning model</strong><span>Apply the routine and walking behavior the user has already taught the app.</span></div><b>→</b><div><strong>Live conditions</strong><span>Re-evaluate transit and traffic without making the user search again.</span></div><b>→</b><div><strong>One intervention</strong><span>Adjust the alarm or departure recommendation only when it materially changes.</span></div></div><p>The ideal daily interaction is deliberately small: the user wakes up, sees the plan, and closes the app. I would measure success by on-time arrival and unnecessary buffer returned—not time spent in the product.</p></CaseSection><CaseSection title="Product in context"><CommuteInUse/><p className="photoCredit">Photography shown here is real, not generated: Anthony Sebbo and Georg Eiermann via Unsplash. The app UI is the portfolio prototype.</p></CaseSection><CaseSection title="What I would validate"><div className="validationGrid"><div><strong>Prediction error</strong><span>Predicted vs. observed arrival time.</span></div><div><strong>Unnecessary buffer</strong><span>How much earlier than the user's target window did they arrive?</span></div><div><strong>Recommendation usefulness</strong><span>Did the suggested route actually change the user's decision?</span></div><div><strong>Interruption quality</strong><span>Did notifications appear only when the user needed to act?</span></div></div><p>The north-star is not engagement. It is getting the user where they need to be on time while returning as much unnecessary morning buffer as possible.</p></CaseSection></>}
...
function Home({openCase}){
 ...
 const featured=['commute','fcvf','accenture'].map(id=>projects.find(p=>p.id===id)).filter(Boolean);
 const more=projects.filter(p=>['scheduler','finsimple','chat','estee'].includes(p.id));
 return <>
