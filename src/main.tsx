...
function SchedulerCase(){return <div className="schedulerStory">
  ...
  <CaseSection title="Research happened in the room" className="schedulerResearchSection"><div className="schedulerFieldNotes"><div className="schedulerParticipants"><strong>7 student conversations</strong>{['N','M','A','J','R','S','K'].map((x,i)=><i key={i}>{x}</i>)}</div><blockquote>Watching students use the working site showed which additions reduced coordination—and which only added interface.</blockquote><div className="schedulerResearchMarks"><span><b>Observed</b>Tentative time was forced into yes/no.</span><span><b>Learned</b>Overlap still needed interpretation.</span><span><b>Changed</b>Preserve uncertainty, then recommend a plan.</span></div></div></CaseSection>
  <CaseSection title="From overlap to a committed plan" className="schedulerDecisionSection"><div className="schedulerCommitment"><div className="schedulerHeatStrip"><i></i><i></i><i className="selected"></i><i></i><i></i></div><div className="schedulerEventCard"><span>Best time</span><strong>Tuesday · 10:30 AM</strong><small>Highest availability · fewest conflicts</small><div><b>Minskoff Pavilion</b><b>3 participants</b><b>Calendar ready</b></div></div><aside><strong>The product closes the decisions after the grid.</strong><p>Recommendation, venue voting, participant status, notes, chat, sharing, and calendar export stay attached to the same event.</p></aside></div></CaseSection>
  <CaseSection title="The event stays synchronized" className="schedulerSystemSection"><div className="schedulerSystemLine"><strong>Browser interaction</strong><i></i><strong>Socket.IO room state</strong><i></i><strong>Flask decisions</strong><i></i><strong>MySQL record</strong></div><p>Docker and Google Cloud Run supported deployment. This portfolio demo preserves the behavior with local browser state so the product remains interactive on GitHub Pages.</p></CaseSection>
  <section className="schedulerOutcome"><strong>Find overlap.</strong><strong>Resolve uncertainty.</strong><strong>Leave with a plan.</strong></section>
</div>}
