...
function KohlerCase(){
 ...
 return <div className="kohlerStory">
  <section className="kohlerProductStage"><KohlerProductSurface/><p className="kohlerPreviewCaption">Switch destinations to see the regional packet adapt. Sample data protects customer, order, and proprietary compliance information.</p></section>

  <CaseSection title="The order at risk" className="kohlerRiskSection"><div className="kohlerRiskLayout"><div className="kohlerOrderIllustration" aria-label="Three steps show a faucet in stock, its missing market documents, and the resulting export hold"><div className="kohlerSkuObject"><b className="kohlerRiskNumber">1</b><span>Faucet in stock</span><div className="kohlerFaucet" aria-hidden="true"><i></i><b></b><em></em></div><strong>K-14402 is ready</strong><small>Physical inventory is confirmed.</small></div><div className="kohlerMissingPacket"><b className="kohlerRiskNumber">2</b><span>Market documents missing</span><div><b>SPEC</b><b>WARRANTY</b><b>LABELS</b></div><strong>The gap</strong><small>India-specific evidence must exist before export.</small></div><div className="kohlerHeldOrder"><b className="kohlerRiskNumber">3</b><span>Order SO-28471</span><div className="kohlerRouteStamp"><b>US</b><i></i><b>IN</b></div><strong>Export is paused</strong><small>The product exists; its market-ready packet does not.</small></div></div><p>When a requested SKU was not configured for its destination market, document readiness—not inventory—could stop the order. Ship Anywhere fills that exact gap: assemble the destination packet, validate it, preserve human control, and retain the evidence behind the decision.</p></div></CaseSection>

  <CaseSection title="People in the workflow" className="kohlerPeopleSection"><div className="kohlerPeopleMap"><div className="kohlerPeopleRoute">{[
  ...
  ].map(([role,job,verb],index)=><article className={`person-${index+1}`} key={role}><b>{index+1}</b><span>{verb}</span><strong>{role}</strong><p>{job}</p></article>)}</div><div className="kohlerPeopleCore"><strong>One packet moves through all four roles</strong><small>Everyone reviews the same status, approved facts, and source trail.</small></div></div></CaseSection>
  ...
  <CaseSection title="Source-of-truth architecture" className="kohlerArchitectureSection"><div className="kohlerArchitectureWrap"><div className="kohlerArchitecture" aria-label="Approved enterprise sources flow through a three-part Azure document workflow into human-reviewable outputs"><div className="kohlerSourceStack"><h3>Approved inputs</h3><article><img src={assetUrl('tool-logos/sap.svg')} alt=""/><div><strong>SAP ECC</strong><small>Order · SKU · destination</small></div></article><article><img src={assetUrl('tool-logos/inriver.webp')} alt=""/><div><strong>Inriver PIM</strong><small>Approved product attributes</small></div></article><article><b>LBL</b><div><strong>Labelling systems</strong><small>Market marks · packaging rules</small></div></article><p>Every fact keeps its source identity.</p></div><div className="kohlerDataCurrent" aria-hidden="true"><i></i><i></i><i></i></div><div className="kohlerAzureCore"><h3>Azure document workflow</h3><p>Three services turn approved data into a packet—without becoming a new source of truth.</p><div className="kohlerAzureSteps"><article><img src={assetUrl('tool-logos/databricks.svg')} alt=""/><div><strong>1. Retrieve trusted data</strong><small>Databricks prepares grounded enterprise context.</small></div></article><article><img src={assetUrl('tool-logos/azure-ai-foundry.svg')} alt=""/><div><strong>2. Compose and check</strong><small>AI Foundry assembles the regional document.</small></div></article><article><img src={assetUrl('tool-logos/copilot-studio.svg')} alt=""/><div><strong>3. Guide human review</strong><small>Copilot Studio presents gaps, checks, and approvals.</small></div></article></div></div><div className="kohlerOutputBranch"><h3>Reviewable results</h3><article><strong>Localized spec sheet</strong><small>Source-linked · versioned</small></article><article><strong>Guided work</strong><small>Checklist · exceptions · review gate</small></article><article><strong>Audit history</strong><small>Inputs · actor · timestamp · output</small></article><p>A person inspects and approves the final packet.</p></div></div><div className="kohlerArchitectureLogic">{[['01','Fetch the order context','Only the SKU, destination, and order identity needed for this request.'],['02','Resolve authority','Product, market, and labelling facts retain source precedence and lineage.'],['03','Compose the regional delta','Stable product content stays fixed while local requirements are assembled.'],['04','Validate before release','Missing or conflicting evidence routes to a person instead of being guessed.']].map(([n,title,copy])=><article key={n}><span>{n}</span><strong>{title}</strong><small>{copy}</small></article>)}</div></div></CaseSection>
  ...
  <CaseSection title="Production delivery" className="kohlerDeliverySection"><div className="kohlerDelivery"><p>We delivered the production workflow as a five-person capstone team. My contribution crossed product definition, interaction design, React and Node implementation, Azure agent orchestration, enterprise-source integration, validation, and release work.</p><div className="kohlerShippedList"><article><strong>Region-specific document generation</strong></article><article><strong>Missing-information and exception handling</strong></article><article><strong>Human review and auditable history</strong></article></div><div className="kohlerReleaseMoment"><div><span>ORDER SO-28471</span><strong>Released for export</strong><small>Validated packet attached · audit record complete</small></div><div className="kohlerFinalRoute" aria-hidden="true"><b>US</b><i><em></em></i><b>IN</b></div><p><span>BEFORE</span><s>Documentation blocks the order.</s><span>AFTER</span><strong>An associate reviews the packet, approves it, and the order continues.</strong></p></div></div></CaseSection>
 </div>
}
