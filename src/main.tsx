...
function ProjectCover({type}){
  ...
  if(type==='accenture') return <div className="editorialCover accentureEditorial"><AccentureWorkflowVisual/></div>;
...
function PhotoCollectionCarousel({section,onExpand}){
  ...
  useEffect(()=>setIndex(0),[section.id]);
  return <section id={section.id} className="photoCollectionSection photoCarouselSection"><div className="photoCollectionMeta"><p>{section.note}</p><h2>{section.title}</h2><span>{section.description}</span></div><div className="photoCarousel" onTouchStart={e=>{touchStart.current=e.touches[0]?.clientX??null}} onTouchEnd={e=>{if(touchStart.current===null)return;const x=e.changedTouches[0]?.clientX??touchStart.current;const delta=x-touchStart.current;if(Math.abs(delta)>45)move(delta<0?1:-1);touchStart.current=null}}><button type="button" className="photoCarouselStage" onClick={()=>onExpand(section.photos[index])} aria-label={`Open ${section.title} photo ${index+1}`}><img key={section.photos[index].src} src={section.photos[index].src} alt={section.photos[index].alt}/></button><div className="photoCarouselControls"><div><button type="button" onClick={()=>move(-1)} aria-label={`Previous ${section.title} photo`}>←</button><button type="button" onClick={()=>move(1)} aria-label={`Next ${section.title} photo`}>→</button></div><span>{String(index+1).padStart(2,'0')} / {String(count).padStart(2,'0')}</span></div><div className="photoFilmstrip" aria-label={`${section.title} thumbnails`}>{section.photos.map((photo,i)=><button type="button" className={i===index?'active':''} key={photo.src} onClick={()=>setIndex(i)} aria-label={`Show ${section.title} photo ${i+1}`} aria-current={i===index?'true':undefined}><img loading="lazy" decoding="async" src={photo.src} alt=""/></button>)}</div></div></section>
}
...
const toolLogoMap={
  ...
  swiftui:{name:'SwiftUI',mark:'UI',urls:[],textOnly:true},
  healthkit:{name:'HealthKit',mark:'♥',urls:[],textOnly:true},
  corelocation:{name:'Core Location',mark:'⌖',urls:[],textOnly:true},
  mapkit:{name:'MapKit / Routes',mark:'Map',urls:[],textOnly:true},
  eventkit:{name:'EventKit / Calendar',mark:'Cal',urls:[],textOnly:true},
  notifications:{name:'UserNotifications / AlarmKit',mark:'Alarm',urls:[],textOnly:true},
  gtfs:{name:'GTFS-Realtime / 511',mark:'511',urls:[],textOnly:true}
};
const toolSets={
  ...
  commute:['swift','swiftui','healthkit','corelocation','mapkit','eventkit','notifications','gtfs','figma','react']
};
...
function ToolLogo({tool}){
 ...
 return <span className={`toolLogoWrap ${failed?'fallback':''} ${safeTool.textOnly?'textOnly':''}`} data-tool={safeTool.name} title={safeTool.name}>{failed?<span className="toolLogoFallback" aria-hidden="true">{safeTool.mark}</span>:<img loading="lazy" decoding="async" src={safeTool.urls[attempt]} alt="" referrerPolicy="no-referrer" onError={()=>setAttempt(v=>v+1)}/>}<span className="toolLogoLabel">{safeTool.name}</span></span>
}
...
function Home({openCase}){
 ...
 <main id="main-content">
  <section id="top" className="hero v28Hero"><div className="heroInner"><h1>Neha Chinimilli</h1><p className="heroThesis">Computer Science + Supply Chain Management at Michigan State University</p><p className="heroTagline">I like products, I like making my life easier and working smarter instead of harder, and I like products that are useful but also look good. I like consulting, I like coding sometimes, but I’d rather be deciding what we build and how we build it than coding all day.</p><div className="heroLinks"><a className="primaryHeroLink" href="#projects">View projects ↓</a><a href="resume.pdf" target="_blank" rel="noreferrer">Resume ↗</a></div></div></section>
