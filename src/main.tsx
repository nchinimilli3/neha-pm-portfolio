...
const metrics = {
  ...
  bookclub: [
    ['1','core weekly loop','product'],
    ['3','member jobs to solve','scope'],
    ['MVP','defined around decisions','artifact']
  ],
  marketExpansion: [
...
const ownership = {
  ...
  bookclub:'I defined the product problem, user journey, MVP scope, and interaction model, then built the concept into a working portfolio prototype.',
...
const projects = [
  ...
  },
  {
    id:'bookclub',
    title:'Bookclub',
    company:'Independent product concept',
    summary:'A calmer home for the decisions that make a book club work: what to read next, when to meet, and how to keep the conversation going between meetings.',
    media:'bookclub',
    facts:['Product strategy','Interactive prototype']
  }
];
...
function AccentureWorkflowVisual(){
  return <div className="accentureVisual workflowCanvas" aria-label="Simplified enterprise AI enablement workflow">
    <div className="workflowTitle"><span>Trainer assignment logic</span><strong>Turn coordinator judgment into inputs, matching logic, and a reviewable recommendation.</strong></div>
    <div className="workflowColumns"><div className="workflowStep"><span>01 · Request</span><strong>Customer, region, topic, timing</strong></div><b aria-hidden="true">→</b><div className="workflowStep"><span>02 · Match</span><strong>Language, expertise, availability, time zone</strong></div><b aria-hidden="true">→</b><div className="workflowStep"><span>03 · Review</span><strong>Reason codes, warnings, human check</strong></div></div>
    <div className="aiEdgeCase"><span>QA finding example:</span><strong>10:30 PM local assignment</strong><small>Working-hours and time-zone constraints became explicit matching requirements.</small></div>
  </div>
}
...
function BookclubPreview(){
 const [picked,setPicked]=useState('Tomorrow, and Tomorrow, and Tomorrow');
 const books=['Tomorrow, and Tomorrow, and Tomorrow','The Midnight Library','The Vanishing Half'];
 return <div className="bookclubPreview">
   <div className="bookclubTop"><span className="bookclubMark">b.</span><div><strong>chapter one</strong><small>your reading circle</small></div><span className="bookclubMembers">8 members</span></div>
   <div className="bookclubNow"><div className="bookSpine"><span>THE<br/>GOLDFINCH</span></div><div><span>Reading now</span><strong>The Goldfinch</strong><p>Part III · 62% complete</p><div className="readingProgress"><i/></div></div></div>
   <div className="bookclubPrompt"><span>Next discussion</span><strong>Thursday · 7:30 PM</strong><p>What did you make of Theo’s choices?</p><button type="button" onClick={()=>setPicked(books[(books.indexOf(picked)+1)%books.length])}>See discussion prompt →</button></div>
   <div className="bookclubVote"><div><span>Choose our next read</span><strong>{picked}</strong></div><button type="button" onClick={()=>setPicked(books[(books.indexOf(picked)+1)%books.length])}>Vote</button></div>
 </div>
}
...
function ProjectCover({type}){
  ...
  if(type==='bookclub') return <BookclubPreview/>;
...
const caseCompanyInfo={
  ...
  bookclub:{name:'Independent app · built for my reading group',logo:''},
...
const toolLogoMap={
  ...
  typescript:{name:'TypeScript',mark:'TS',urls:['https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/typescript.svg','https://cdn.simpleicons.org/typescript/3178C6']},
  ...
  ucl:{name:'UCL component library',mark:'UCL',urls:['tool-logos/ucl.svg']},
  rally:{name:'Rally',mark:'Rally',urls:['tool-logos/rally.svg']},
  graphapi:{name:'Graph API validation',mark:'Graph API',urls:['tool-logos/graphapi.svg']},
  ...
  swiftui:{name:'SwiftUI',mark:'UI',urls:['tool-logos/swiftui.svg']},
  ...
  mapkit:{name:'MapKit / Routes',mark:'Map',urls:['tool-logos/mapkit.svg']},
  eventkit:{name:'EventKit / Calendar',mark:'Calendar',urls:['tool-logos/eventkit.svg']},
  notifications:{name:'UserNotifications / AlarmKit',mark:'Alerts',urls:['tool-logos/notifications.svg']},
  gtfs:{name:'GTFS-Realtime / 511',mark:'511',urls:['tool-logos/gtfs.svg']},
  ...
  weatherkit:{name:'WeatherKit / weather context',mark:'Weather',urls:['tool-logos/weatherkit.svg']},
  ...
  d1:{name:'Cloudflare D1',mark:'D1',urls:[],textOnly:true},
  r2:{name:'Cloudflare R2',mark:'R2',urls:[],textOnly:true},
  ...
  nyt:{name:'The New York Times Books',mark:'NYT',urls:[],textOnly:true},
  applebooks:{name:'Apple Books',mark:'Books',urls:[],textOnly:true},
  googlevision:{name:'Google Vision OCR',mark:'OCR',urls:[],textOnly:true}
};
const toolSets={
  ...
  bookclub:['react','typescript','vite','cloudflare','d1','r2','googlecalendar','nyt','applebooks','openai','googlevision'],
...
function ToolLogo({tool}){
 ...
 return <span className={`toolLogoWrap ${failed?'fallback':''} ${safeTool.textOnly?'textOnly':''}`} data-tool={safeTool.name} tabIndex={0} aria-label={safeTool.name}>{failed?<span className="toolLogoFallback" aria-hidden="true">{safeTool.mark}</span>:<img loading="lazy" decoding="async" src={safeTool.urls[attempt]} alt="" referrerPolicy="no-referrer" onError={()=>setAttempt(v=>v+1)}/>}<span className="toolLogoLabel" aria-hidden="true">{safeTool.name}</span></span>
}
function ToolLogoStrip({id}){const items=toolSets[id]||[];if(!items.length)return null;const description=id==='fcvf'?'Tools and technologies used across the FCVF build.':id==='finsimple'?'Tools and platform technologies used in the FinSimple work.':id==='bookclub'?'React product surface with Cloudflare Workers, D1, and R2 supporting the club experience.':'Tools used in this project.';return <section className="toolLogoSection"><div className="toolLogoHeading"><h2>Tech stack</h2><p>{description}</p></div><div className="toolLogoRow">{items.map(k=><ToolLogo key={k} tool={toolLogoMap[k]}/>)}</div></section>}
...
function CaseStudy({id,onBack}){
 ...
 return <main className={`casePage case-${id}`} onClick={openClickedImage}><AuraField tone={id}/><button className="backBtn" onClick={onBack}>← Projects</button><section className="caseLead"><header className="caseHeader"><CaseCompanyBar id={id} fallback={p.company}/><h1>{p.title}</h1><div className="caseIntro">{p.summary}</div>{id!=='commute'&&id!=='finsimple'&&<div className="ownershipLine"><span>{ownership[id]}</span></div>}{id!=='commute'&&metrics[id]&&<MetricStrip items={metrics[id]}/>}</header><div className="caseHeroMedia casePreviewHero"><ProjectVisual type={p.media}/></div></section><ToolLogoStrip id={id}/>
...
function BookclubCase(){return <>
  <CaseSection title="The problem"><p>Book clubs do not usually fail because people do not like books. They lose momentum because choosing the next one, reading at different speeds, deciding when to meet, and keeping track of the conversation all happen in separate places.</p><p>I built Bookclub for my friends and me: one private space that protects the small decisions and moments that make a reading group feel worth returning to.</p></CaseSection>
  <CaseSection title="The loop I designed"><div className="bookclubLoop"><div><span>01</span><strong>Gather</strong><p>Create a private club or join with an invite.</p></div><div><span>02</span><strong>Choose</strong><p>Search, nominate, and vote on a small shortlist.</p></div><div><span>03</span><strong>Read together</strong><p>Set a pace, share progress, and save thoughts safely.</p></div><div><span>04</span><strong>Meet + remember</strong><p>Coordinate the discussion, rate the read, and grow the shared shelf.</p></div></div></CaseSection>
  <CaseSection title="What I prioritized"><div className="factGrid"><Fact title="A private room, not a public feed">Each club is invite-only, so the product can serve a real friend group without asking members to perform for an audience.</Fact><Fact title="Preference over popularity">Members can say “want to read,” “would read,” or “not this time.” That creates a clearer signal than one loud favorite and gives a workable path through runoffs or ties.</Fact><Fact title="Different reading speeds, one conversation">Chapter/page progress and checkpoints create context for discussion. Reading Room posts, quotes, questions, and predictions can stay anchored to where readers are in the book.</Fact><Fact title="Coordination stays lightweight">Instead of starting with a heavy calendar grid, an admin offers a few realistic times, members tap what works, and the group can RSVP or add the decision to a calendar.</Fact></div></CaseSection>
  <CaseSection title="The reading experience"><div className="bookclubEvidence"><figure><img loading="lazy" decoding="async" src="project-media/bookclub-preview.png" alt="Bookclub's finished-reading shelf, showing books the group has completed together"/><figcaption>A shared shelf preserves the club’s finished reads.</figcaption></figure><div className="bookclubEvidenceCopy"><span>Built for the in-between moments</span><h3>Keep the thought; avoid the spoiler.</h3><p>The Reading Room supports reactions, replies, saved meeting questions, quotes, predictions, and quick passage capture. The point is not to turn reading into another noisy feed—it is to give the group a place to hold onto the moments they want to discuss.</p><ul><li>Update progress by chapter or page</li><li>Create a reading plan and checkpoints</li><li>Save questions for the next meeting</li><li>Rate the book and preserve it on the club shelf</li></ul></div></div></CaseSection>
  <CaseSection title="What I built"><div className="bookclubBuildGrid"><div><strong>Discovery + selection</strong><span>Book search and browse, nominations, preference voting, runoff handling, and a shared shortlist.</span></div><div><strong>Shared reading</strong><span>Reading plans, checkpoints, progress tracking, spoiler-aware discussion, passage capture, and meeting-question capture.</span></div><div><strong>Meeting rhythm</strong><span>Time polls, RSVP states, calendar handoff, scheduled reminders, ratings, and an archive shelf that carries the group into its next pick.</span></div></div></CaseSection>
  <CaseSection title="Architecture"><p>React, TypeScript, and Vite power the product experience. Cloudflare Workers handle server-side integrations, AI/OCR workflows, calendar actions, and scheduled reminders; Cloudflare D1 stores the club’s relational data, while R2 holds product media and assets.</p><div className="bookclubArchitecture"><span>React app</span><b>↔</b><span>Cloudflare Workers</span><b>↔</b><span>D1</span><b>+</b><span>R2</span></div><p className="diagramNote">Integrated services: Google Calendar, NYT Books, Apple Books, OpenAI, and Google Vision OCR.</p></CaseSection>
  <CaseSection title="What would tell me it works"><div className="bookclubMeasures"><div><strong>Selection momentum</strong><span>Members nominate and finish votes instead of defaulting to a group chat stalemate.</span></div><div><strong>Reading participation</strong><span>Members update progress, reach checkpoints, and leave thoughts while the book is active.</span></div><div><strong>Discussion quality</strong><span>Questions, quotes, and predictions make it into the meeting rather than disappearing in a text thread.</span></div><div><strong>Club continuity</strong><span>The group rates a finished book and starts a new shortlist for the next one.</span></div></div></CaseSection>
</>}
function MarketExpansionCase(){return <>
  <CaseSection title="Decision context"><p>A consumer services company wanted a clearer way to evaluate future locations while also strengthening revenue generation at existing branches. The decision mixed market opportunity with operational realities: a promising area still needed the right customer profile, partnership ecosystem, facility conditions, and competitive landscape.</p><p>The consulting team researched the business and branch context. I owned the interactive Excel scorecard and its supporting rubric, turning that research into a repeatable decision system rather than a one-time recommendation.</p></CaseSection>
  <CaseSection title="From research to a decision tool"><div className="marketMethod"><div><span>1</span><strong>Define the decision</strong><p>Separate what made a market attractive from what made a location workable.</p></div><div><span>2</span><strong>Structure the evidence</strong><p>Translate qualitative research into criteria that could be evaluated consistently.</p></div><div><span>3</span><strong>Weight the criteria</strong><p>Make strategic importance explicit instead of treating every input as equal.</p></div><div><span>4</span><strong>Compare options</strong><p>Score candidate locations through the same rubric and expose the tradeoffs.</p></div></div></CaseSection>
  <CaseSection title="Criteria and weighting"><p>I organized the model around four decision areas: strategic partnerships, facility specifications, customer demographics, and competition intensity. The supporting rubric defined what stronger and weaker evidence looked like for each input, while adjustable weights let the team reflect changing priorities without rebuilding the tool.</p><div className="marketCriteria"><div><strong>Strategic partnerships</strong><span>Complementary businesses, institutions, and event demand that could create repeatable customer channels.</span></div><div><strong>Facility fit</strong><span>Operating requirements such as space, rent, and infrastructure needed to support the concept.</span></div><div><strong>Customer demographics</strong><span>Income, population, and local demand indicators connected to the target customer.</span></div><div><strong>Competition intensity</strong><span>Direct and adjacent alternatives that could constrain demand or signal market fit.</span></div></div></CaseSection>
  <CaseSection title="Location comparison"><p>The same model was applied to three anonymized candidate markets. Two emerged as closely matched lead options, while the third scored lower and became a later-stage opportunity. The value was not the ranking alone: decision-makers could see which criteria created the difference and update assumptions as new information arrived.</p><div className="marketComparison"><div className="lead"><span>Location A</span><strong>123</strong><small>Lead option</small></div><div><span>Location B</span><strong>122</strong><small>Close second</small></div><div><span>Location C</span><strong>101</strong><small>Explore later</small></div></div></CaseSection>
  <CaseSection title="Branch growth recommendations"><p>Alongside the expansion tool, the consulting team diagnosed seasonal demand softness and recommended three practical growth levers for existing branches.</p><div className="marketRecommendations"><div><strong>Community outreach</strong><span>Build local awareness through relevant events and community relationships.</span></div><div><strong>B2B partnerships</strong><span>Create recurring demand through complementary organizations and event-driven partners.</span></div><div><strong>Targeted paid media</strong><span>Use location and audience targeting to reach new customer segments and measure campaign performance.</span></div></div></CaseSection>
  <CaseSection title="What success would look like"><div className="marketMeasures"><div><strong>Decision consistency</strong><span>Every candidate is evaluated against the same defined criteria and rubric.</span></div><div><strong>Transparent tradeoffs</strong><span>Leaders can see why one location ranks above another and challenge the underlying assumptions.</span></div><div><strong>Reusable analysis</strong><span>The team can update weights and inputs as strategy, costs, and market evidence change.</span></div><div><strong>Actionable growth plan</strong><span>Existing branches leave with prioritized channels to test, not a list of generic ideas.</span></div></div></CaseSection>
</>}
...
function Home({openCase}){
 ...
 const serious=['fcvf','accenture','finsimple','scheduler','marketExpansion'].map(id=>projects.find(p=>p.id===id)).filter(Boolean);
 ...
 <header className="siteHeader"><a className="wordmark" href="#top">Neha Chinimilli</a><nav aria-label="Primary"><a href="#projects">Projects</a><a href="#experience">Experience</a><a href="#fun">Fun builds</a><a href="resume.pdf" target="_blank" rel="noreferrer">Resume</a></nav></header>
