...
export default function FinSimpleMachE(){
 ...
 useEffect(()=>{
  ...
  const update=()=>{
   ...
   const p=reduced.matches?0:Math.max(0,Math.min(1,(window.innerHeight*.8-rect.top)/(window.innerHeight*.45)));
   const ease=p*p;
 ...
 return <div className="meScrollScene" ref={scene}><div className="meRoadStage"><div className="meElectricWake" ref={streak} aria-hidden="true"><i/><i/><i/></div><div className="meScrollCar" ref={car}><MachEArtwork/></div><div className="meGroundLine" aria-hidden="true"/></div></div>
}
